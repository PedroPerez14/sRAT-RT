...
private:
    ...
    void set_shader_camera_uniforms(Shader* shader, Camera* cam, int width, int height);
    ...
    void render_quad()             // The deferred lighting pass itself
    {
...
public:
    ...
    void draw(GLFrameBuffer<INTERNAL_FORMAT, FORMAT, N_TEXTURES>* framebuffer, Camera* cam)
    {
        ...
        for(Model& model : m_models)
        {
            ...
            set_shader_camera_uniforms(model.get_deferred_shader(), camera, framebuffer->getWidth(), framebuffer->getHeight());        /// TODO
        ...
        {
            /// TODO: Do the uniforms later
            set_shader_camera_uniforms(model.get_forward_shader(), camera, framebuffer->getWidth(), framebuffer->getHeight()); /// TODO
